...
sampling_dict["noise_specs"] = {'type':'phase_amplitude_damping_error', 'max_prob': 0.25}
...
for mode, rounds in zip(["train", "test"], [train_rounds, test_rounds]):
    ...
    prob_path = qcd.pool_shuffle_split(os.getcwd(), 'run_prob_%s' % mode, mode=mode, split=1, delete=False)
    adjT_path = qcd.pool_shuffle_split(os.getcwd(), 'run_adjT_%s' % mode, mode=mode, split=1, delete=False)
