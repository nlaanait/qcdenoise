...
def get_data():
    data = {'1': [(1, 2, 1.0)],
            ...
            '8': [(1, 2, 1.0), (2, 3, 1.0), (3, 4, 1.0), (4, 5, 1.0),
                  (5, 1, 1.0)],
            '9': [(1, 2, 1.0), (1, 3, 1.0), (1, 4, 1.0), (1, 4, 1.0),
                  (1, 6, 1.0)],
            '10': [(1, 6, 1.0), (2, 6, 1.0), (3, 6, 1.0), (4, 5, 1.0),
                   (5, 6, 1.0)],
            '11': [(1, 6, 1.0), (2, 6, 1.0), (3, 5, 1.0), (4, 5, 1.0),
                   (5, 6, 1.0)]
            }
    ...
    check_list = [0, 1, 1, 2, 5, 2, 0, 0, 0, 0]

    return data, grouped_data, data_part_1, data_part_2, check_list


@ pytest.mark.dependency()
def test_graph_data(get_data):
    data, grouped_data, _, _, _ = get_data
    graph_data = qcd.GraphData(
        data=data)
    # test attributes
...
def test_graph_partition(get_data):
    data, _, part_1, part_2, _ = get_data
    graph_data = qcd.GraphData(data=data)
    graph_data_1, graph_data_2 = qcd.partition_GraphData(
...
def test_graph_db(get_data):
    data, _, _, _, check_list = get_data
    graph_data = qcd.GraphData(data=data)
    graph_db = qcd.GraphDB(graph_data=graph_data)
    assert list(graph_db.keys()) == list(data.keys())
    sorted_graphs = graph_db.get_edge_sorted_graphs()
    sorted_lengths = [len(itm) for _, itm in sorted_graphs.items()]
    assert sorted_lengths == check_list
