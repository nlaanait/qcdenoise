...
def pool_shuffle_split(files_dir, file_expr,
    ...
    runs = np.concatenate(
        [np.load(os.path.join(files_dir, file), mmap_mode='r') for file in files])
    for _ in range(5):
...
def prob_adjT_to_lmdb(lmdb_path, prob_data_path,
                      adjT_data_path, lmdb_map_size=int(10e9), delete=True):
    env = lmdb.open(
        lmdb_path,
...
class QCIRCDataset(Dataset):
    ...
    def __init__(self, lmdb_path, pads=(0, 0, 0, 0), input_transform=None, target_transform=None,
        ...
        with self.env.begin(write=False) as txn:
            input_shape = np.frombuffer(
                txn.get(b"input_shape"), dtype='int64')
            target_shape = np.frombuffer(
                txn.get(b"target_shape"), dtype='int64')
            encoding_shape = np.frombuffer(
                txn.get(b"encoding_shape"), dtype='int64')
            input_dtype = np.dtype(
                txn.get(b"input_dtype").decode("ascii"))
            target_dtype = np.dtype(
                txn.get(b"target_dtype").decode("ascii"))
            encoding_dtype = np.dtype(
                txn.get(b"encoding_dtype").decode("ascii"))
            ...
            self.num_samples = int(
                txn.get(b"num_samples").decode("ascii"))
        ...
        self.input_keys = [
            bytes(
                self.data_specs['input_key'] +
                str(idx),
                "ascii") for idx in self.records]
        self.target_keys = [
            bytes(
                self.data_specs['target_key'] +
                str(idx),
                "ascii") for idx in self.records]
        self.encoding_keys = [
            bytes(
                self.data_specs['encoding_key'] +
                str(idx),
                "ascii") for idx in self.records]
        self.print_debug(
            "Opened lmdb file %s, with %d samples" %
            (self.lmdb_path, self.num_samples))
    ...
    def __getitem__(self, idx):
        ...
        inputs = np.frombuffer(input_bytes,
                               dtype=self.data_specs['input_dtype'])
        inputs = inputs.reshape(self.data_specs['input_shape'])
        if inputs.dtype != np.float32:
            inputs = inputs.astype(np.float32)
        targets = np.frombuffer(target_bytes,
                                dtype=self.data_specs['target_dtype'])
        targets = targets.reshape(self.data_specs['target_shape'])
        if targets.dtype != np.float32:
            targets = targets.astype(np.float32)
        encodings = np.frombuffer(
            encoding_bytes,
            dtype=self.data_specs['encoding_dtype'])
        encodings = encodings.reshape(
            self.data_specs['encoding_shape'])
        ...
        return {'input': torch.from_numpy(inputs), 'target': torch.from_numpy(
            targets), 'encoding': encodings}  # pylint: disable=no-member
...
class QCIRCDatasetMulti(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        assert idx >= 0 and idx < sum(
            self.db_records), print(
            idx, sum(
                self.db_records))
        # fetch records
        # try:
        with self.db[db_idx].begin(write=False, buffers=True) as txn:
            ...
            self.print_debug(
                'going into lmdb file %s, reading %d' %
                (self.lmdb_path[db_idx], idx))
        ...
        return {'input': torch.from_numpy(
            input), 'target': torch.from_numpy(target)}
