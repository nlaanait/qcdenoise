...
def gatePropToGate(props, random=False):
    ...
    gate = Gate(gate=props["gate"], name=props["name"], parameters=parameters, qubits=props["qubits"])
    return gate
...
class UnitaryNoiseSpec:
    def __init__(self):
        self.specs = {'type':'phase_amplitude_damping_error', 'max_prob':0.1} 
            ...
            if qubits:
                if len(qubits) == 1:
                    ...
                    if gate_name not in gate_specs.keys():
                        gate_specs[gate_name] = {"gate":gate.name, "qubits":[q_idx]}
                    plane_idx = 0
                    write_success = False
                ...
                if len(qubits) == 2:
                    ...
                    gate_name = "%s_%d_%d" %(gate.name, q_idx_1, q_idx_2)
                    if gate_name not in gate_specs.keys():
                        gate_specs[gate_name] = {"gate":gate.name, "qubits":[q_idx_1, q_idx_2]}
                    plane_idx = 0
                    write_success = False
                    while plane_idx < adj_tensor.shape[0]:
...
class UnitaryNoiseSampler(CircuitSampler):
    def __init__(self, *args, noise_specs=UnitaryNoiseSpec().specs, **kwargs):
...
class DeviceNoiseSampler(CircuitSampler):
    def __init__(self, *args, noise_specs=DeviceNoiseSpec().specs, **kwargs):
    ...
    def build_noise_model(self, ideal=False):
        ...
        # generate a dictionary of gate/qubit propeties and transform into qiskit's BackendProperties
        ...
        self.print_verbose("qubits:\n{}\n".format(self.backend_props["qubits"]))
        self.print_verbose("gates:\n{}\n".format(self.backend_props["gates"]))
        ...
        for qbit, q_prop in zip(self.circuit.qubits, q_props):
            for error_props in q_prop:
                if error_props["name"] == "prob_meas1_prep0":
                    zero_weights = [1 - error_props["value"], error_props["value"]]
                elif error_props["name"] == "prob_meas0_prep1":
                    one_weights = [error_props["value"], 1 - error_props["value"]] 
    ...
    def get_adjacency_tensor(self, max_tensor_dims=(16, 32, 32), basis_gates=['id','cx','u1','u2','u3'], 
        ...
        for q_idx, props in enumerate(self.backend_props["qubits"]):
            idx = 0
            for prop in props:
                self.print_verbose("encoding %s at (%d, %d, %d)" % (prop["name"], idx, q_idx, q_idx))
        ...
        for props in self.backend_props["gates"]:
            ...
            for prop in props:
                if len(indices) == 1:
                    ...
                    self.print_verbose("encoding %s at (%d, %d, %d)" % (prop["name"], idx, q_idx_1, q_idx_2))
    ...
    def generate_qbit_props(self):
        ...
        for name in self.noise_specs["qubit"].keys():
            if name in ["T1", "T2"]:
                ...
                ent = {"date":date, "name":name, "unit": "µs", "value":value}
            else:
                value = self.get_random_value(name, "qubit")
                ent = {"date":date, "name":name, "unit": "", "value":value}
            dicts.append(ent)
        return dicts
