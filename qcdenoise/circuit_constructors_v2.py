...
class CircuitConstructor:
    ...
    def __init__(self,n_qubits=2, n_shots=1024, verbose=False,
        ...
        if self.backend==None and state_simulation==False:
            ...
            if self.backend_name==None:
                raise ValueError("Using noisy simulator requires backend_name")
            if self.backend_name not in fake_devices.keys():
                raise ValueError("Unrecognized name, here are the options: "+\
                "Boebligen\nBogota\nCambridge\nEssex\nJohannesburg\nMelbourne\n"+\
                "Poughkeepsie\nTenerife\nRueschlikon\nSingapore\nTokyo")
            if fake_devices[self.backend_name].configuration().n_qubits<self.n_qubits:
                raise ValueError("not enough qubits in backend")
            self.device = qk.providers.aer.QasmSimulator.from_backend(fake_devices[self.backend_name])
    ...
    def construct_diagonals(self):
        ...
        b = [1.,-1.]
        diags = {}
        if self.stab_circuits==None:
            self.build_stabilizer_circuits()
        for idx in range(len(self.stab_circuits)):
            stab_sgn,stab_ops = sgn_convert[self.stab_circuits[idx].name[0]],list(self.stab_circuits[idx].name[1:])
            ...
            while len(stab_ops_temp)>0:
                ...
                d = [a if pop_op=='I' else b][0]
                D = np.kron(d,D)
            diags[self.stab_circuits[idx].name]=np.multiply(stab_sgn,D)
    ...
    def get_stabilizer_measurements(self):
        ...
        if self.track_gates==True:
            if self.hardware_qubits!=None:
                ...
                self.gate_count[hw_tup]=self.gate_count.get(hw_tup,[])
        ...
        for idx in range(len(self.stab_circuits)):
            ...
            stab_exp_vals[self.stab_circuits[idx].name]=igmit.expectation_value(test_counts,diagonal=dvec)
    ...
    def get_witness_measurements(self,witness_list,coef_list=[]):
        ...
        if self.diags==None:
            self.construct_diagonals()
        witness_circuits = [self.stab_circuits[x] for x in witness_list]
        ...
        for idx in range(len(witness_circuits)):
            ...
            op_exp,op_var = igmit.expectation_value(test_counts,diagonal=dvec)
            operator_measurements.append(op_exp)
            operator_variances.append(op_var)
        return operator_measurements,operator_variances,np.sum(np.multiply(coef_list,operator_measurements))
...
class GraphCircuit(CircuitConstructor):
    ...
    def __init__(self, graph_data=None, gate_type="Hgate_CZ",
        ...
        if (gate_type=='CPHASE'):
            if ('rot_angle' not in kwargs.keys()):
                print('building controlled phase gates with default rotation pi/2')
    ...
    def build_circuit(self):
        if self.circuit_graph==None:
            print('call build graph first')
            return
        else:
            # 3. Build a circuit from the graph state
            if self.gate_type == "Hgate_CZ":
                self.print_verbose("Assigning a Decomposed CZ gate (H-CNOT-H) to Node Edges")
                self._build_Hgate_CZ_gate()
            if self.gate_type == "CPHASE":
                self.print_verbose("Assigning a Controlled Phase Gate to Node Edges with rotation angle",str(self.rot_angle))
    ...
    def get_biseparable_witnesses(self):
        ...
        generators are n-length strings (n = number of qubits)"""
        if self.stabilizer_measurements==None:
            self.get_stabilizer_measurements()
        bs_witness = {}
        for idx,edx in enumerate(self.circuit_graph.edges()):
            e0,e1=edx
            gi = [k for k in self.stabilizer_measurements.keys() if k[::-1][e0]=='X'][0]
            gj = [k for k in self.stabilizer_measurements.keys() if k[::-1][e1]=='X'][0]
            val = 1 - self.stabilizer_measurements[gi][0]-self.stabilizer_measurements[gj][0]
            meas_vals = [1.0,-1.*self.stabilizer_measurements[gi][0],-1.0*self.stabilizer_measurements[gj][0]]
            std_vals=[0.0,self.stabilizer_measurements[gi][1],self.stabilizer_measurements[gj][1]]
            arr = unumpy.uarray(meas_vals,std_vals)
            #print('W: ',val,unumpy.nominal_values(arr.sum()).tolist())
            #print(unumpy.std_devs(arr.sum()).tolist())
            if self.hardware_qubits==None:
                    bs_witness[idx]=[tuple([e0,e1]),val,'TBD',self.noise_robust,self.backend_name,tuple(())]

            elif self.hardware_qubits!=None:
        ...
        bisep_wit_df = pd.DataFrame.from_dict(bs_witness,orient='index')
        if len(bisep_wit_df.columns)==0:
            self.bisep_witnesses = bisep_wit_df
        else:
            bisep_wit_df.columns=['W(i,j)','value','var','noise_robust_level','backend','qubits']
            self.bisep_witnesses = bisep_wit_df
    ...
    def get_genuine_witnesses(self):
        ...
        if self.noise_robust==0:
            if self.stabilizer_measurements==None:
                self.get_stabilizer_measurements()
            df = pd.DataFrame.from_dict(self.stabilizer_measurements,orient='index')
            df.columns=['meas','var']
            W_ = (self.circuit_graph.order()-1)*(df['meas'].loc['+'+'I'*self.n_qubits]) \
                - (df['meas'].sum() - df['meas'].loc['+'+'I'*self.n_qubits])
            #print('propagation of error not yet implemented, someone should do that')
            nl=len(self.stabilizers)
            ns=len([x for x in self.stabilizers if x!='+'+'I'*self.n_qubits])
            wit_coef = np.multiply(np.ones(nl),-1.0)
            iden_idx = self.stabilizers.index('+'+'I'*self.n_qubits)
            ...
            #print(unumpy.std_devs(arr.sum()).tolist())
            self.genuine_witness=(W_,unumpy.std_devs(arr.sum()).tolist(),self.noise_robust)
    ...
    def build_unique_stabilizer_circuits(self):
        # Build all stabilizer circuits from the graph state and append stabilizer measurements
        if self.circuit_graph==None:
            print('build graph first')
            return
        if self.gate_type == "Controlled_Phase":
            self.print_verbose("Assigning a decomposed Controlled Z Gate (H-CNOT-H) to Node Edges")
            ...
            self.print_verbose("Assigning a Controlled Phase gate to Node Edges")
    ...
    def _build_Bvertices(self):
        trials = {}
        # this is a kludge -- it will need to be refined as the graphs get bigger
        for tries in range(5*self.circuit_graph.order()):
            ...
            for idx in range(len(test_set)):
                ...
                if len(start)==0:
                    ...
                    s0.append(start[0])
                    d2_vert = d2_neighborhood(self.circuit_graph,start[0])
                    test_set = test_set - d2_vert
            trials[tries]=test_set
        min_len = np.min([len(v) for k,v in trials.items()])
        max_len = np.max([len(v) for k,v in trials.items()])
        min_set_B = np.unique(np.asarray([tuple(v) for k,v in trials.items() if v.__len__()==min_len]),axis=0)
        max_set_B = np.unique(np.asarray([tuple(v) for k,v in trials.items() if v.__len__()==max_len]),axis=0)
        self.min_set_B = {min_len:min_set_B}
        self.max_set_B = {max_len:max_set_B}
        self.B_set = self.max_set_B[max_len][np.random.choice(range(len(self.max_set_B[max_len])))]
    ...
    def _build_decomposable_witness(self):
        ...
        max_count=0
        if (self.min_set_B == None) and (self.max_set_B == None):
            self._build_Bvertices
    ...
    def _build_unique_Hgate_CZ_stabilizer(self):
        ...
        for sdx in get_unique_operators(self.stabilizers):
            ...
            if type(self.circuit_graph)==nx.DiGraph:
                ...
                stab_ops = list(sdx)
                circ.barrier()
                circ=build_stabilizer_meas(circ,sdx,drop_coef=False)
                if self.state_sim:
...
class GHZCircuit(CircuitConstructor):
    ...
    def build_biseparable_witnesses(self):
        ...
        for idx in range(1,self.n_qubits):
            idx_gen = [x for x in self.generators if x[idx-1:idx+1]=='ZZ'][0]
    ...
    def build_genuine_witnesses(self,noise_robust=0):
        ...
        if noise_robust==0:
            ...
            coefs = np.multiply(-1.0,np.ones(len(self.generators)+1))
            ...
            for idx in range(1,self.n_qubits):
                ...
                Sm_wit = [x for x in range(len(self.stab_circuits)) if \
                            self.stab_circuits[x].name=='+'+idx_gen][0]
                ops_list.append(Sm_wit)
            # add in products of generators to improve noise_robustness
